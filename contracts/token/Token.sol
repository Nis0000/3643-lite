...
contract Token is IToken, AccessControl, Pausable {
    ...
    /// @param account The address to check.Fset
    ...
    function _transfer(address from, address to, uint256 amount) private {
        uint16 code=_identityRegistry.investorCountry(msg.sender);
        ...
        require(
            _compliance.canTransfer(from, to, amount,code),
    ...
    function _mint(address account, uint256 amount) private {
        ...
        require(
            _compliance.canTransfer(address(0), account, amount,code),
            "ERC-3643: Compliance failure"
        );
